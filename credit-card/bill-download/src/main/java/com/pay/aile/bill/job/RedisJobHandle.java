...
public class RedisJobHandle {
    ...
    public void addJob(CreditEmail creditEmail) {
        ...
        // // 如果是已经存在的
        logger.info("添加到任务队列中{},isDownload{}", creditEmail.getEmail(), creditEmail.isDownload());
        redisTemplate.opsForValue().set(MAIL_DOWANLOD_JOB_CONTENT.concat(id), JSONObject.toJSONString(creditEmail));

        if (creditEmail.isDownload() || creditEmail.getIsNew()) {
    ...
    public CreditEmail getContent(String id) {
        String jsonstring = redisTemplate.opsForValue().get(MAIL_DOWANLOD_JOB_CONTENT.concat(id));
        if (!org.springframework.util.StringUtils.hasText(jsonstring)) {
    ...
    public CreditEmail getJob() {
        ...
        String key = MAIL_DOWANLOD_JOB_CONTENT.concat(id);
        logger.info("key--------------{}", key);
