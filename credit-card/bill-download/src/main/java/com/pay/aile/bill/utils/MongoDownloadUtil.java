...
public class MongoDownloadUtil {
    ...
    public EmailFile getFile(String fileName, int month) {
        try {

            EmailFile ef = mongoTemplate.findOne(new Query(Criteria.where(DOC_KEY_FILE_NAME).is(fileName)),
    ...
    public void saveCreditFile(List<CreditFile> creditFileList, CreditNativeEmail creditEmail) {
        creditFileRelation.saveNotExitsCreditFile(creditFileList, creditEmail);
    }
    public void saveEmailFiles(List<EmailFile> emailFileList) {
        emailFileList.forEach(e -> {
            logger.info("mongofile，months========================={}", e.getSentDate().substring(4, 6));
    ...
    public String saveFile(EmailFile emailFile, CreditFile creditFile) {
        logger.info("调用saveFile1=============================={}", creditFile.getId());
    ...
    public void saveFile(List<EmailFile> emailFileList, List<CreditFile> creditFileList, CreditNativeEmail creditEmail) {
        // logger.info("调用saveFile2=============================={}",
        // emailFileList.size());
        saveEmailFiles(emailFileList);
        saveCreditFile(creditFileList, creditEmail);
