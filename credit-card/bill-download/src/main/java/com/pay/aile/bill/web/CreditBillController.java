        ...
        try {
            ...
            eamilList.forEach(e -> {
                ...
                if (System.currentTimeMillis() - creditEmail.getLastJobTimestamp() >= refreshSeconds * 1000) {
                    ...
                    // 初始化
                    redisTemplate.opsForValue().set(Constant.REDIS_ANALYSIS_STATUS + email.getEmail()
                            + Constant.EMAIL_USERID_SEPARATOR + email.getUserId(), "0");
                    // credit_card_analyzed_status_+email+userId
                    redisTemplate.opsForValue().set(Constant.REDIS_ANALYZED_STATUS + email.getEmail()
                            + Constant.EMAIL_USERID_SEPARATOR + email.getUserId(), "0");
